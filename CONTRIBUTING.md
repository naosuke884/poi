...
The design notes in [`docs/internals.ja.md`](./docs/internals.ja.md) (Japanese) explain how the board editor,
the `/api/board` contract, the expiry cron and the offline handling work — worth skimming before changing them.
...
- If you change the database schema, follow "スキーマを変更するとき" in `docs/internals.ja.md`
  and commit the generated migration in `drizzle/`. Do not hand-edit `worker/db/schema.ts` (generated by Better Auth).
