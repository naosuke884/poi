...
tmp=$(mktemp)
trap 'rm -f "$tmp"' EXIT
if ! curl -sSfL --retry 3 -o "$tmp" "$url"; then
  echo "::error file=README.md::README の埋め込み動画 ($url) を取得できません。手元で ./scripts/sync-demo-video.sh を実行し、差し替わった README.md をコミットしてください" >&2
  exit 1
fi
