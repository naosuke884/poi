...
# 一致の判定は check-demo-video.sh に任せる。既存 URL が取得できない場合も不一致としてアップロードに進む
if ./scripts/check-demo-video.sh >/dev/null 2>&1; then
  echo "OK: README の埋め込み動画と public/demo.mp4 は既に一致しています"
  exit 0
fi
