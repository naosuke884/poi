...
使い方・セットアップ・デプロイ手順は [README](../README.ja.md) を参照。
このドキュメントは、コードを読む / 変更する人向けに設計と挙動の詳細をまとめたもの。

TanStack Router (SPA) + Hono (API) + Better Auth (Google ログイン) + D1 + Mantine を
1 つの Cloudflare Worker で動かす構成。

アプリは「板」1 枚: ログインすると「セクション」が縦に並び、書いた内容は自動保存される。
編集中のセクションだけ Textarea で、それ以外は Markdown をレンダリングして表示する。
...
│   ├── main.tsx              # エントリ (MantineProvider + createRouter。defaultErrorComponent = RouteErrorFallback)
│   ├── routes/               # file-based routing
│   │   ├── __root.tsx        #   AppShell レイアウト + ヘッダー (ロゴ / SaveStatusIcon / UserMenu)
│   │   ├── index.tsx         #   /          板 (要ログイン: beforeLoad でガード)。オフライン時はキャッシュを閲覧のみで表示
│   │   └── login.tsx         #   /login     (Google でログイン)
│   ├── components/UserMenu.tsx
│   ├── components/Board.tsx  # 板 (セクションごとの Textarea / Markdown 表示との切り替え / 分割・結合・移動のキー操作 / debounce 自動保存 / オフライン時は保持して復帰後に再送)
│   ├── components/MarkdownView.tsx      # 編集していないセクションの Markdown 表示 (react-markdown + remark-gfm + remark-breaks。記法は一部だけ有効)
│   ├── components/MarkdownView.module.css # 見出し・箇条書きの見た目
│   ├── components/SaveStatusIcon.tsx    # ヘッダーの保存状態アイコン (雲)。offline / error はクリックで再試行
...
│   ├── lib/save-status.ts    # 板の保存状態を Board → ヘッダー (SaveStatusIcon) に渡す小さなストア (useSyncExternalStore)
│   ├── lib/markdown-disable.ts # Markdown 記法を名前で無効化する remark プラグインと、板で無効にする記法の一覧
...
│   ├── db/index.ts           # createDb(env.DB): Drizzle (D1) インスタンス
...
- セクション (= `memo` 1 行) を縦に並べる (Notion のブロックのような構造)。見た目は 1 枚の文書で、
  セクションの境界は期限ラベル「あと N 日で消えます」(未保存なら「新しいセクション」) と削除ボタン付きの区切り線で示す。
  板が空なら空のセクションを 1 つ出す
- 編集中 (フォーカスのある) セクションだけ `Textarea` で、それ以外は `MarkdownView` で Markdown をレンダリングして表示する。
  開いた直後はすべて Markdown 表示で、クリック / タップで Textarea に切り替わり、フォーカスが外れると Markdown 表示に戻る
  (ウィンドウ自体がフォーカスを失った場合は戻さない)。内容はそのまま Markdown テキストとして保存する
  - 使える記法は「# 見出し」「- 箇条書き / 1. 番号付き」と URL の自動リンクだけ。強調・コード・引用・表・HTML などは
    `src/lib/markdown-disable.ts` (`BOARD_MARKDOWN_DISABLED`) で構文ごと無効化し、書いた文字がそのまま表示される
    (メモ用途で誤発動しやすい記法を切っている。HTML も構文ごと無効なので sanitize は不要)
  - 改行 1 つはそのまま改行 (`remark-breaks`)。リンクは別タブで開く
...
- 保存状態はヘッダーの雲アイコン (`SaveStatusIcon`。`src/lib/save-status.ts` のストア経由) に出す:
  未保存 / 保存中 / 保存済み / 保存に失敗 (クリックで再試行) /
  オフライン (クリックで再試行。`online` イベントでも自動再送。下記「オフライン時の挙動」)。
  未保存の間はタブを閉じる・リロード時に `beforeunload` で確認を出す
- 上限は `worker/memo/constants.ts` の `BOARD_MAX_LENGTH` (`boardLength` = 区切りを含めた板全体の長さ) と
  `BOARD_MAX_SECTIONS`。超えると保存せずエラー表示
...
`main` への push (と手動の `workflow_dispatch`) で `npm ci` → `typecheck` → `build` → `db:migrate:remote` → `wrangler deploy`
(`cloudflare/wrangler-action`) を実行する。job 全体が `if: vars.ENABLE_DEPLOY == 'true'` でガードされているので、
fork しただけでは何も起きない (スキップ扱い)。有効化の手順は [README](../README.ja.md#github-actions-からデプロイする)。

- `concurrency: deploy-production` で直列に走らせ、`cancel-in-progress: false` にしている
  (途中キャンセルすると「D1 マイグレーション適用済み・Worker 未デプロイ」で止まりうるため)
- `workflow_dispatch` を他ブランチから起動しても `github.ref == 'refs/heads/main'` の条件で本番には出ない
- Worker の Secrets (`BETTER_AUTH_SECRET` など) は `wrangler secret put` で登録済みのものが使われる。GitHub 側には
  `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` だけ置く
...
3. `npm run db:migrate:local` で手元に適用して確認する。本番は `deploy.yml` がデプロイ前に自動で適用する
   (手元からデプロイする場合は `npm run db:migrate:remote` を先に実行する)
